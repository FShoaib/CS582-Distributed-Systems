...
PAXOS_NODE=$GOPATH/src/paxosapp/runners/prunner/prunner.go
...
# Start paxos node.
go run -race ${PAXOS_NODE} -myport=${NODE_PORT0} -ports=${ALL_PORTS} -N=5 -id=0 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID0=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT1} -ports=${ALL_PORTS} -N=5 -id=1 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID1=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT2} -ports=${ALL_PORTS} -N=5 -id=2 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID2=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT3} -ports=${ALL_PORTS} -N=5 -id=3 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID3=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT4} -ports=${ALL_PORTS} -N=5 -id=4 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID4=$!
sleep 1

# Start paxostest.
${PAXOS_TEST} -port=${TESTER_PORT} -paxosports=${ALL_PORTS} -N=5 -nodeport=${NODE_PORT0} -pxport=${PROXY_PORT} -pidkill=${PAXOS_NODE_PID1} -initwait=10 -repwait=20 -repretries=10 -timeout=100
